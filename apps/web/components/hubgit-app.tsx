...
export function HubGitApp({ initialPath = '/' }: { initialPath?: string }) {
  ...
  useEffect(() => {
    ...
    if (params.get('brand') === 'github-reference') setBranding('github-reference');
    if (params.get('viewer') === 'guest') setViewer('guest');
...
function AuthPage({ mode, branding, setBranding, onLogin }: { mode: string; branding: Branding; setBranding: (v: Branding) => void; onLogin: () => void }) {
  ...
  const submit = (event: FormEvent) => { event.preventDefault(); onLogin(); window.location.href = '/dashboard'; };
  return (
...
function RefsPage({ kind }: { kind: string }) { return <FeatureList heading={kind === 'tags' ? 'Tags' : 'Branches'} icon={kind === 'tags' ? <TagIcon size={20} /> : <BranchIcon size={20} />} items={kind === 'tags' ? ['v0.1.0 — HubGit preview', 'v0.0.2 — API foundation'] : ['main — Default', 'feature/provider-contract — Active', 'docs/fidelity-matrix — Active']} />; }
...
function ProjectsPage() { return <section><div className="feature-heading"><h1>Projects</h1><button className="primary-button">New project</button></div><div className="project-board">{[['Backlog', issues.slice(2)], ['In progress', issues.slice(0, 2)], ['Done', issues.slice(3)]].map(([column, cards]) => <section key={String(column)}><h2>{String(column)} <span>{(cards as typeof issues).length}</span></h2>{(cards as typeof issues).map(([title, meta]) => <article key={title}><IssueOpenedIcon size={15} /><strong>{title}</strong><small>{meta}</small></article>)}</section>)}</div></section>; }
