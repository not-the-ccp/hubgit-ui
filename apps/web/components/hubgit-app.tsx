...
/** Safe, local-only defaults used by the mock application. */
export const MOCK_LOCAL_AUTH_PROFILE: AuthProfile = {
  id: 'mock-local',
  label: 'HubGit local account',
  mode: 'local',
};

/** Example provider profile: it can only begin an OAuth-style redirect. */
export const GITHUB_PROVIDER_AUTH_PROFILE: AuthProfile = {
  id: 'github-provider',
...
export const BRANDING_PRESETS = {
  hubgit: HUBGIT_BRANDING,
  'github-reference': GITHUB_REFERENCE_BRANDING,
} as const;
...
const issues = [
  [
    'Add keyboard navigation to the ref selector',
    '#42 opened 2 hours ago by octocat',
    ['accessibility', 'frontend'],
  ],
  [
    'Large unified diffs lose the selected line after virtualization',
    '#38 opened yesterday by monalisa',
    ['bug', 'diffs'],
  ],
  [
    'Support capability discovery for provider adapters',
    '#31 opened 3 days ago by hubot',
    ['api', 'enhancement'],
  ],
  [
    'Document the Forgejo adapter conformance suite',
    '#27 opened last week by not-the-ccp',
    ['documentation'],
  ],
] as const;
...
const pulls = [
  [
    'Build provider-neutral repository overview payload',
    '#57 opened 3 hours ago by monalisa',
    'Checks passing',
  ],
  [
    'Add split and unified diff layouts',
    '#54 opened yesterday by octocat',
    'Review required',
  ],
  [
    'Create deterministic workflow run fixtures',
    '#49 opened 4 days ago by hubot',
    '1 check failing',
  ],
] as const;

const navPath = (suffix = '') =>
  `/not-the-ccp/hubgit-ui${suffix ? `/${suffix}` : ''}`;
...
export function HubGitApp({
  ...
  const [viewer, setViewer] = useState<Viewer>(
    initialPath === '/login' || initialPath === '/join' ? 'guest' : 'member',
  );
  ...
  return (
    <div className={`hubgit-app ${dark ? 'theme-dark' : ''}`}>
      ...
      {path === '/pulls' ? (
        <GlobalQueue title="Pull requests" kind="pull" />
      ) : null}
      {path === '/notifications' ? <Notifications /> : null}
      {path.startsWith('/search') ? (
        <SearchResults query={query || 'adapter API'} />
      ) : null}
  ...
  return (
    <header className="global-header">
      ...
      <div className="header-actions">
        ...
        <a className="icon-button" href="/issues" aria-label="Issues">
          <IssueOpenedIcon size={17} />
        </a>
        <a className="icon-button" href="/pulls" aria-label="Pull requests">
        ...
        {viewer === 'guest' ? (
          ...
          <a className="avatar" href="/not-the-ccp" aria-label="Open profile">
            M
          </a>
        )}
      </div>
    ...
    <main className="auth-page">
      ...
      <section className="auth-card">
        ...
        {isProvider ? (
          ...
          <>
            <p>
              A verification link for this local demo was sent to{' '}
              <strong>monalisa@hubgit.test</strong>.
            </p>
            <button className="primary-button wide" type="button">
              Open development mailbox
            </button>
          </>
        ) : (
          <form onSubmit={submit}>
            ...
            {mode === 'join' && (
              <>
                ...
                <input id="name" defaultValue="monalisa" />
                <label htmlFor="email">Email address</label>
                <input
                  id="email"
                  type="email"
                  defaultValue="monalisa@hubgit.test"
                />
              </>
            )}
            {mode !== 'password-reset' && mode !== 'two-factor' && (
              <>
            ...
            {mode === 'two-factor' ? (
              ...
              <>
                <label htmlFor="password">
                  {mode === 'password-reset' ? 'Email address' : 'Password'}
                </label>
                <input
                  id="password"
                  name="password"
  ...
  return (
    <main className="dashboard-layout page-width">
      <aside className="dashboard-sidebar">
        ...
        {[
          'not-the-ccp/hubgit-ui',
          'lcaml-foundation/caramel',
          'not-the-ccp/avelune',
          'lcaml-foundation/ouroboros',
        ].map((repo) => (
          <a className="repo-list-link" href={`/${repo}`} key={repo}>
            <RepoIcon size={16} />
            {repo}
          </a>
        ))}
        <h3>Your teams</h3>
        <a className="repo-list-link" href="/orgs/lcaml-foundation">
          <PeopleIcon size={16} />
          lcaml-foundation/core
        </a>
      </aside>
      <section className="feed-column">
        ...
        <FeedCard
          ...
          target="not-the-ccp/hubgit-ui"
        />
        <FeedCard
          actor="octocat"
          action="opened pull request #57 in"
          target="not-the-ccp/hubgit-ui"
        />
        <FeedCard
          actor="hubot"
          action="released v0.1.0 in"
          target="not-the-ccp/hubgit-ui"
        />
      </section>
      <aside className="dashboard-aside">
        <section>
          ...
          <p>
            <time>2 hours ago</time> Visual regression baseline added for
            repository pages.
          </p>
          <p>
            <time>Yesterday</time> New API capability matrix is available.
          </p>
          <a href="/">View changelog →</a>
        </section>
        <section>
          <h2>Explore repositories</h2>
          <strong>primer/react</strong>
          <p>The React implementation of GitHub&apos;s design system.</p>
          <span>TypeScript · 3.9k stars</span>
        </section>
      </aside>
    </main>
  );
...
function FeedCard({
  ...
  return (
    <article className="feed-card">
      <div className="avatar small">{actor[0].toUpperCase()}</div>
      <div>
        <p>
        ...
        <div className="feed-repo">
          ...
          <strong>{target}</strong>
          <span>Public</span>
          <p>Provider-neutral Git collaboration frontend.</p>
          <div>
  ...
  return (
    <>
      ...
      <nav className="repo-navigation" aria-label="Repository navigation">
        {repoTabs.map(([label, segment, Icon, count]) => (
          <a
            className={
      ...
      <main className="repo-page page-width">
        ...
        {active === 'pulls' &&
          (suffix.match(/^pulls\/\d+/) ? <PullDetail /> : <PullList />)}
    ...
    <div className="repo-grid">
      <section>
        <div className="content-toolbar">
          ...
          <button>
            Add file <ChevronDownIcon size={12} />
          </button>
          <button className="primary-button">
            &lt;&gt; Code <ChevronDownIcon size={12} />
          </button>
        </div>
  ...
  return (
    <article className="readme-panel">
      ...
      <div className="readme-content">
        ...
        <h2>HubGit UI</h2>
        <p>
          A provider-neutral, near-pixel-perfect GitHub frontend for self-hosted
          Git servers.
        </p>
        ...
        <h3>What is this?</h3>
        <p>
          HubGit recreates the familiar Git collaboration experience with an
          original implementation, adapter-ready API, and deterministic local
          backend.
        </p>
        <pre>
...
function IssueList() {
  return (
    <ListPage
      ...
      items={issues.map(([title, meta, labels]) => ({
        title,
        meta,
        labels: [...labels],
        icon: <IssueOpenedIcon className="state-open" size={16} />,
        href: navPath('issues/42'),
      }))}
    />
  );
}
function PullList() {
  return (
    <ListPage
      title="Pull requests"
      createLabel="New pull request"
      search="is:pr is:open"
      items={pulls.map(([title, meta, status]) => ({
        title,
        meta,
        labels: [status],
        icon: <GitPullRequestIcon className="state-open" size={16} />,
        href: navPath('pulls/57'),
      }))}
    />
  );
}
    ...
    <section className="list-page" aria-label={title}>
      <div className="list-actions">
        <div className="query-builder">
          <FilterIcon size={16} />
          <input aria-label={`Search ${title}`} defaultValue={search} />
        </div>
        <button>Labels</button>
        <button>Milestones</button>
        <button className="primary-button">{createLabel}</button>
      </div>
      <div className="list-panel">
        <div className="list-panel-header">
          ...
          <span>Closed</span>
          <span className="toolbar-spacer" />
          Author <ChevronDownIcon size={12} /> Label{' '}
          <ChevronDownIcon size={12} /> Sort <ChevronDownIcon size={12} />
        </div>
        {items.map((item) => (
          <article className="work-item" key={item.title}>
            ...
            <span className="comment-count">
              <CommentDiscussionIcon size={16} /> 3
            </span>
          </article>
        ))}
      </div>
  ...
  return (
    <section className="detail-page">
      <div className="detail-heading">
        ...
        <button className="primary-button">
          New {kind === 'issue' ? 'issue' : 'pull request'}
        </button>
        <p>
          ...
          monalisa opened this {kind} 2 hours ago · 3 comments
        </p>
      </div>
      {kind === 'pull' && (
        <nav className="detail-tabs">
          <a className="active" href="/">
            <CommentDiscussionIcon size={16} /> Conversation 3
          </a>
          <a href="/">
            <GitCommitIcon size={16} /> Commits 2
          </a>
          <a href="/">
            <CheckCircleFillIcon size={16} /> Checks 5
          </a>
          <a href="/">
            <DiffIcon size={16} /> Files changed 8
          </a>
        </nav>
      )}
      <div className="conversation-grid">
        <div className="timeline">
          <Comment
            author="monalisa"
            body="This change introduces a single repository overview resource and keeps provider-specific response details behind the adapter ports."
          />
          <div className="timeline-event">
            <GitCommitIcon size={16} />
            <strong>monalisa</strong> added 2 commits
          </div>
          <Comment
            author="octocat"
            body="The response shape looks good. I left one suggestion around capability flags and pagination."
          />
          {kind === 'pull' && (
            <div className="merge-box">
              ...
              <div>
                ...
                <p>2 approving reviews · branch is up to date</p>
              </div>
              <button className="primary-button">
                <GitMergeIcon size={16} /> Merge pull request
              </button>
            </div>
          )}
        </div>
        <aside className="metadata-sidebar">
          <Metadata title="Assignees" value="monalisa" />
          <Metadata title="Labels" value="api · enhancement" />
          <Metadata title="Projects" value="HubGit 1.0" />
          <Metadata title="Milestone" value="Foundation" />
          <Metadata title="Development" value="2 linked branches" />
        </aside>
      </div>
    </section>
  );
...
function Comment({ author, body }: { author: string; body: string }) {
  return (
    <article className="comment">
      <div className="avatar small">{author[0].toUpperCase()}</div>
      <div>
        <header>
          <strong>{author}</strong> commented 2 hours ago{' '}
          <KebabHorizontalIcon size={16} />
        </header>
        <div>
          <p>{body}</p>
          <pre>
            <code>{`GET /api/v1/repos/{owner}/{repo}/overview?ref=main`}</code>
          </pre>
          <button>
            <SmileyIcon size={16} /> 4
          </button>
        </div>
      </div>
    </article>
...
function CommitHistory() {
  return (
    <FeatureList
      ...
      items={[
        'Build the first complete HubGit product slice',
        'Add provider-neutral OpenAPI models',
        'Seed pull requests and workflow runs',
        'Create responsive repository shell',
      ]}
    />
  );
}
  ...
  return (
    <section>
      <div className="feature-heading">
        <div>
          <h1>Build the first complete HubGit product slice</h1>
          <p>Authored and committed by monalisa 2 hours ago</p>
        </div>
        <code>a21bd771e53</code>
      </div>
      <div className="diff-summary">
        <strong>8 files changed</strong>
        <span className="additions">+427</span>
        <span className="deletions">−18</span>
      </div>
      {['apps/web/app/page.tsx', 'apps/api/app/main.py'].map((file) => (
        <article className="diff-file" key={file}>
          <header>
            <DiffIcon size={16} />
            <strong>{file}</strong>
            <span className="toolbar-spacer" />
            Viewed <ChevronDownIcon size={12} />
          </header>
          <pre>
            <span>@@ -1,4 +1,9 @@</span>
            {`\n`}
...
function RefsPage({ kind }: { kind: string }) {
  return (
    <FeatureList
      heading={kind === 'tags' ? 'Tags' : 'Branches'}
      icon={
        kind === 'tags' ? <TagIcon size={20} /> : <GitBranchIcon size={20} />
      }
      items={
        kind === 'tags'
          ? ['v0.1.0 — HubGit preview', 'v0.0.2 — API foundation']
          : [
              'main — Default',
              'feature/provider-contract — Active',
              'docs/fidelity-matrix — Active',
            ]
      }
    />
  );
...
function ReleasesPage() {
  return (
    <FeatureList
      ...
      items={[
        'HubGit preview 0.1.0 · Latest',
        'Contract foundation 0.0.2',
        'Initial clean-room skeleton 0.0.1',
      ]}
    />
  );
}
function DiscussionsPage() {
  return (
    <FeatureList
      ...
      items={[
        'Welcome to HubGit Discussions',
        'How should provider capabilities map to navigation?',
        'Show and tell: custom instance themes',
      ]}
    />
  );
}
function ActionsPage() {
  return (
    <FeatureList
      ...
      items={[
        'CI · main · passing',
        'Visual regression · pull/57 · in progress',
        'Publish containers · v0.1.0 · passing',
      ]}
      statuses
    />
  );
  ...
  return (
    <section>
      ...
      <div className="project-board">
        {columns.map(({ title: column, cards }) => (
          <section key={column}>
            <h2>
              {column} <span>{cards.length}</span>
            </h2>
  ...
  return (
    <section className="wiki-layout">
      <article className="readme-content">
        <h1>HubGit Wiki</h1>
        <p>Welcome to the project knowledge base.</p>
        <h2>Architecture</h2>
        <p>
          The browser consumes only the provider-neutral API. Adapters translate
          external Git-host data into the stable HubGit domain.
        </p>
        <h2>Pages</h2>
        <ul>
          <li>Getting started</li>
          <li>Adapter authoring</li>
          <li>Fidelity workflow</li>
        </ul>
      </article>
      <aside>
  ...
  return (
    <FeatureList
      ...
      items={[
        ...
        'Code scanning · 2 passing analyses',
      ]}
      statuses
    />
  );
      ...
      <div className="insight-grid">
        {[
          ...
          <article key={item}>
            <GraphIcon size={24} />
            <h2>{item}</h2>
            <strong>{index === 0 ? '24 updates' : `${12 + index * 7}`}</strong>
...
function BlobPage() {
  const code = [
    'import { createApiClient } from "@hubgit/contracts";',
    '',
      ...
      <div className="feature-list">
        {items.map((item, index) => (
          <article key={item}>
            {statuses ? (
              index === 1 ? (
                <XCircleFillIcon className="state-closed" size={18} />
              ) : (
                <CheckCircleFillIcon className="state-open" size={18} />
              )
            ) : (
              icon
            )}
            <div>
              <strong>{item}</strong>
              <p>
                Updated {index + 1} day{index ? 's' : ''} ago by monalisa
              </p>
            </div>
...
function GlobalQueue({
  ...
  return (
    <main className="global-queue page-width">
      <h1>{title}</h1>
...
function Notifications() {
  return (
    <main className="notifications-layout page-width">
      <aside>
        <h2>Inbox</h2>
        <a className="active" href="/">
          Inbox <span>4</span>
        ...
        <a href="/">Done</a>
        <h3>Repositories</h3>
        <a href="/">not-the-ccp/hubgit-ui</a>
        <a href="/">primer/react</a>
      </aside>
      <section>
        <div className="feature-heading">
          <h1>Notifications</h1>
        ...
        <FeatureList
          ...
          items={[
            'Review requested: provider-neutral overview payload',
            'Issue assigned: keyboard navigation',
            'Workflow completed: CI on main',
            'New release: HubGit preview 0.1.0',
          ]}
        />
      </section>
    </main>
...
function SearchResults({ query }: { query: string }) {
  return (
    <main className="search-layout page-width">
      <aside>
        <h2>Filter by</h2>
        {[
          'Code',
          ...
          <a className={index === 0 ? 'active' : ''} href="/" key={filter}>
            {filter}
            <span>{12 - index}</span>
          </a>
        ))}
      ...
      <section>
        ...
        <FeatureList
          ...
          items={[
            ...
            'apps/web/features/repositories/api.ts',
  ...
  return (
    <main className="profile-page page-width">
      <aside>
        ...
        <h1>Monalisa Octocat</h1>
        <span>not-the-ccp</span>
        <p>Building delightful developer tools and clean-room interfaces.</p>
        <button>Edit profile</button>
        <p>
          <PeopleIcon size={16} /> <strong>128</strong> followers ·{' '}
          <strong>42</strong> following
        </p>
        <p>
          <OrganizationIcon size={16} /> lcaml-foundation
        </p>
      </aside>
      <section>
        <nav className="profile-tabs">
          ...
          <a href="/">Repositories 18</a>
          <a href="/">Projects</a>
          <a href="/">Stars 96</a>
        </nav>
        <h2>Popular repositories</h2>
        <div className="pinned-grid">
          {['hubgit-ui', 'avelune', 'git-sha1-cuda', 'voice-thingie'].map(
            (repo) => (
              <article key={repo}>
                <a href={repo === 'hubgit-ui' ? navPath() : '#'}>
                  <strong>{repo}</strong>
                </a>
                <span>Public</span>
                <p>Open-source experiments and useful developer tooling.</p>
                <small>
                  <i /> TypeScript · ★ 18
                </small>
              </article>
        ...
        <h2>312 contributions in the last year</h2>
        <div className="contribution-grid">
          {Array.from({ length: 182 }, (_, i) => (
            <i key={i} data-level={(i * 7 + 3) % 5} />
          ))}
        </div>
      </section>
    </main>
...
function OrganizationPage() {
  return (
    <main className="organization-page page-width">
      <header>
        <div className="org-avatar">LC</div>
        <div>
          <h1>LCaml Foundation</h1>
          <p>Open-source tools for language and compiler developers.</p>
          <a href="/">lcaml.org</a>
        </div>
      </header>
      <nav className="profile-tabs">
      ...
      <div className="org-grid">
        <section>
          ...
          <div className="pinned-grid">
            {['caramel', 'ouroboros', 'lccolor', 'stdlib'].map((repo) => (
              <article key={repo}>
                <a href="/">
                  <strong>{repo}</strong>
                </a>
                <span>Public</span>
                <p>Production-ready open source from LCaml Foundation.</p>
              </article>
            ))}
          </div>
        </section>
        <aside>
          <h2>People</h2>
          <div className="avatar-stack">MOHLCAS</div>
          <a href="/">View all members</a>
          <h2>Top languages</h2>
          <p>OCaml · Rust · TypeScript</p>
        </aside>
      </div>
    </main>
  );
...
function SettingsPage({ scope }: { scope: 'user' | 'repository' }) {
  const groups =
    scope === 'user'
      ? [
          'Public profile',
  ...
  return (
    <main className="settings-layout page-width">
      <aside>
        <h2>
          {scope === 'user' ? 'Personal settings' : 'Repository settings'}
        </h2>
      ...
      <section>
        ...
        <div className="settings-section">
          <h2>
            {scope === 'user' ? 'Profile information' : 'Repository name'}
          </h2>
          <label>
            Display name
            <input
              defaultValue={scope === 'user' ? 'Monalisa Octocat' : 'hubgit-ui'}
            />
          </label>
          <label>
            Bio
            <textarea defaultValue="A provider-neutral Git collaboration frontend." />
          </label>
          <button className="primary-button">Save changes</button>
        </div>
        <div className="settings-section">
          <h2>Features</h2>
          {['Issues', 'Discussions', 'Projects', 'Wiki', 'Pipelines'].map(
            (feature) => (
              <label className="check-row" key={feature}>
                <input type="checkbox" defaultChecked />{' '}
                <strong>{feature}</strong>
                <span>Enable the {feature.toLowerCase()} module.</span>
              </label>
            ),
          )}
        </div>
          ...
          <div className="danger-zone">
            ...
            <div>
              <span>
                ...
                <small>Make the repository read-only.</small>
              </span>
            ...
            <div>
              <span>
                ...
                <small>Permanently remove repository data.</small>
              </span>
